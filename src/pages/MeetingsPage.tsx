  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {showCreateForm && (
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-gray-900 dark:text-white font-medium">{t('meetings.createTitle')}</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">{t('meetings.form.title')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">{t('meetings.form.platform')}</label>
                <select
              ...
              <div>
                <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">{t('meetings.form.date')}</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">{t('meetings.form.time')}</label>
                <input
                  type="time"
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-sm ${newMeeting.recordWithAi ? 'text-white' : 'text-gray-400'}`}>
                    {t('meetings.recordWithAi')}
                  </span>
                </div>
              </label>
              {newMeeting.recordWithAi && (
                <span className="text-xs text-gray-500">
                  {t('meetings.recordWithAiHint')}
                </span>
              )}
            </div>
        ...
        {/* Meeting list */}
        {isLoading ? (
          <p className="text-gray-500 text-center py-8">{t('meetings.loading')}</p>
        ) : (
          <MeetingList
            meetings={meetings}
